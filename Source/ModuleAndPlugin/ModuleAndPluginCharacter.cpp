...
void AModuleAndPluginCharacter::SetupPlayerInputComponent(UInputComponent* PlayerInputComponent)
{
	...
	if (UEnhancedInputComponent* EnhancedInputComponent = Cast<UEnhancedInputComponent>(PlayerInputComponent)) {
		...
		EnhancedInputComponent->BindAction(MouseLookAction, ETriggerEvent::Triggered, this, &AModuleAndPluginCharacter::Look);
	...
	{
		UE_LOG(LogModuleAndPlugin, Error, TEXT("'%s' Failed to find an Enhanced Input component! This template is built to use the Enhanced Input system. If you intend to use the legacy system, then you will need to update this C++ file."), *GetNameSafe(this));
	}
}
...
void AModuleAndPluginCharacter::DoMove(float Right, float Forward)
{
...
void AModuleAndPluginCharacter::DoLook(float Yaw, float Pitch)
{
	if (GetController() != nullptr)
	{
...
void AModuleAndPluginCharacter::DoJumpStart()
{
	// signal the character to jump
	Jump();
}

void AModuleAndPluginCharacter::DoJumpEnd()
{
